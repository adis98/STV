...
if __name__ == "__main__":
    data = pd.read_csv('RossmanSales/train.csv')
    STORE_ID = 1
    data = data.groupby(['Store'])
    data = data.get_group(STORE_ID)
    data = data[::-1]
    data = data.drop(columns=['Date', 'Store'])
    data = pd.get_dummies(data)
    temp = data['StateHoliday_0']
    temp['Sum'] = temp.sum(axis=1)
    data['StateHoliday_o'] = data['StateHoliday_0'].sum(axis=1)
    data.drop(columns=['StateHoliday_0'], inplace=True)
    data['StateHoliday_0'] = data['StateHoliday_o']
    data.drop(columns=['StateHoliday_o'], inplace=True)
    ...
    for winsz in [50, 100, 200, 400]:
        ...
        print("Prequential Window size:", winsz, "MSE SARIMAX NE:", mean(mse_sarimax_ne), "MSE SARIMAX MLE:",
              ...
              "MSE SARITRAX:", mean(mse_saritrax))
