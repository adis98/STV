...
class VerticalXGBoostTree:
    ...
    def setMapping(self):
        """This creates a randomized mapping between the global feature ID (randomized) with the local one, i.e., 0..num_features-1 for this client"""
        rand = np.random.permutation(self.data.shape[1]) # Get column number
        if self.rank == 0:
            return
        if len(self.featureList) == 0:
            # This if statement is some bullshit never executes
    ...
    def fit(self, y_and_pred, tree_num):
        ...
        # the rank 2 process partitions the shuffled feature indices and distributes these to the clients.
        #for e.g. 5 features: 0,1,2,3,4 may be partitioned as [1,3]--> rank 1, [0,2,4] --> rank 2
        if self.rank == 2:
            random_list = np.random.permutation(self.featureNum)  # returns a random permutation as list from 0 to num_features-1
            start = 0
        ...
        if self.rank == 1: # Calculate gradients on the node who have labels.
            y, y_pred = self._split(y_and_pred)  # this literally just splits the true label from the predicted one
